...
Prior to starting assignment 5, we wanted to tidy up our previous data and add a new variable to explore.We also clarified more restrictive definition of bikeway infrastructure, by excluding sharrows and other infrastructure that is not a separated bike lane from vehicular traffic.  

## Rerun multivariate analysis

Then with our new variables, pop_density and percent_bike, we decided to run a new multivariate analysis (full_model2), which yielded a statistically significant negative correlation of pop density and bike lane length. Every additional linear foot of bike lane, the average census tract looses .085 people per square mile. The R squared explains 8.6% variation in sum length while our old R-square (full_model) (without the new variables) showed 2.1 % variation in sum length, so our new model is a better fit.
...
```
After we log transformed population density and reran the regression, log_pop_density retained its significance and in addition med_income_E also became statistically significant. The R square value tells how much variation in dependent variable is due to the other variables. Our R square is .1779 for the log_transformation, which is up from .086. This means that our model now explains 17.79% of the variation in the dependent variable, and is rounded to 18% in the cleaner table.
